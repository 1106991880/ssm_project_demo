	...
	$('#menuGrid').datagrid({
		...
		toolbar: [{
				...
				handler: function(){
					...
					if(menu != null){
						setMenuValues(menu);
						$.messager.alert('温馨提示', '该功能后台已实现，现在不允许你乱修改!', 'info');
						//openEdit('编辑菜单', 'edit');
					}else{
...
function deleteMenu(menuId){
	$.messager.confirm('温馨提示', '确认要删除这条记录？', function(r){
	    if(r){
	    	var url = '<%=path%>/menu/deleteMenuById';
				...
				$.messager.alert('温馨提示', '该功能后台已实现，现在不允许你乱删除!', 'info');
	...
	function loadIcons() {
		var iconNames = [ 'man', 'book', 'note', 'money', 'organisation',
				'menu', 'role', 'home', 'globe', 'exit', 'cabin', 'download',
				'forward', 'backward', 'notice', 'clock', 'feed' ];
		var icons = [];
				...
				.combobox(
						{
							...
							formatter : function(row) {
								return '<div class="'+row.value+' md"><span style="margin-left: 25px;">'
										+ row.text + '</span></div>';
							}
						});
	}
...
<body>
	<fieldset class="fieldset-self">
		...
		<table>
			<tr>
				...
				<td align="left"><input type="text" id="menuNameCon"
					class="easyui-searchbox" data-options="searcher:queryData" />
				</td>
			</tr>
		</table>
	</fieldset>
	...
	<div id="menuDialog" class="easyui-dialog" data-options="closed: true"
		style="padding: 10px;">
		<form id="menu_form" method="POST">
			...
			<table class="table_info">
				<tr>
					...
					<th>菜单图标：</th>
					<td><input type="text" id="menuIcon" name="menuIcon"
						style="width: 150px;"></td>
				</tr>
				<tr>
					<th>菜单地址：</th>
					<td colspan="5"><input type="text" id="menuUri" name="menuUri"
						class="easyui-textbox" style="width: 650px;"
						data-options="required:true" missingMessage="菜单地址必须填写"></td>
				</tr>
				<tr>
					<td colspan="6"><b>菜单描述：</b><br> <input type="text"
						id="menuDesc" name="menuDesc" class="easyui-textbox"
						data-options="multiline:true" style="width: 730px; height: 100px;">
					</td>
				</tr>
			</table>
		</form>
